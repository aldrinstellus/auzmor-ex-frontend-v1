...
function fn_showEmojiPalette(quill: {
  ...
  ele_emoji_area.style.top = `${
    paletteTop > 260 ? 260 : paletteTop + quillContainerCord.top
  }px`;
    ...
    if (customButton) {
      customButton.addEventListener('click', () => {
        ...
        setTimeout(() => {
          ...
          quill.container.parentNode.scrollTop = quill.getBounds(
            range.index + 1,
          ).top;
        }, 0);
        fn_close();
