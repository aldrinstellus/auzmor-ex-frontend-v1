...
const EventWidget: FC<IEventWidgetProps> = ({ className = '' }) => {
  ...
  const eventsAttendees = attendees?.data?.result?.data?.map((attendee: any) => ({
    id: attendee.id,
        ...
        <Button
          ...
          onClick={() => {
            window.location.assign(
              `${getLearnUrl()}/user/trainings?type=events&tab=${isLive ? 'ONGOING' : 'UPCOMING'}`
            );
          }}
      ...
      <Card className="mt-2 w-full relative overflow-hidden">
        {(() => {
          if (events && events?.length > 0 && !isLoading) {
            return (
              <div className={`w-full ${isLive ? 'h-[397px]' : 'h-[353px]'} relative overflow-hidden group/card`}>
                <img
                  src={event?.image_url}
                ...
                <div className={`absolute ${isLive ? 'bg-primary-500' : 'bg-white border-neutral-200'} z-10 gap-1 flex top-4 left-4 px-2.5 py-1 text-xs font-medium rounded`}>
                  <Icon
                    name={isLive ? 'radar' : 'videoPlay'}
                    size={18}
                    color={isLive ? 'text-white' : 'text-primary-500'}
                  />
                  <p className={`text-xs ${isLive ? 'text-white' : 'text-primary-500'} font-semibold`}>
                    {isLive ? t('live') : <TimeChip startDate={startDate} userTimeZone={userTimezone} />}
                  </p>
                </div>
                <div className="absolute bg-white bottom-0 left-0 flex flex-col p-4 z-10 gap-2 w-full">
                  <div className="flex gap-1">
                    {event?.categories?.slice(0, 2)?.map((d: any) => (
                      <div key={d?.id} className="px-2 py-1 rounded bg-primary-100 text-center text-primary-500 text-xs font-medium">
                        {d?.title}
                      </div>
                    ))}
                  ...
                  <div className="flex items-center gap-2">
                    <Avatar name={event?.created_by?.full_name} image={event?.created_by?.image_url} size={32} />
                    ...
                    <div className="flex ml-auto gap-1 items-center">
                      <Icon name={event.location ? 'location' : 'video'} size={20} color="text-primary-500" />
