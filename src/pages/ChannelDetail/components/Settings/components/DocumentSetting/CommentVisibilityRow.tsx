...
const CommentVisiblityRow: FC<AppProps> = ({ data, canEdit }) => {
  const { t } = useTranslation('channelDetail', {
    keyPrefix: 'setting.documentSetting.commentVisibilityRow',
  });
  ...
  const handleChange = (canComment: boolean) => {
    updateChannelMutation.mutate({
      channelId,
      settings: {
        restriction: { canComment },
      },
    });
  };
  return (
    <InfoRow
      ...
      isEditMode={true}
      value={!!data.settings?.restriction?.canComment}
      dataTestId=""
      editNode={
        <SwitchToggle
          defaultValue={!!data.settings?.restriction?.canComment}
          onChange={(checked) => {
...
export default CommentVisiblityRow;
